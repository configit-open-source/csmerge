...
namespace CsMerge.Core {
  ...
  public class PackageReference: Package {
    ...
    public PackageReference( string id,
      ...
      bool? userInstalled = null ) : base(id,version){
    ...
    public static IEnumerable<PackageReference> Read( string path ) {
      if ( !File.Exists( path ) ) {
    ...
    public static void Write( IEnumerable<PackageReference> packages, string path ) {
      using ( var fw = new StreamWriter( path ) ) {
        Write( packages, fw );
      }
    }

    public static IEnumerable<PackageReference> Parse( string content ) {
      if ( string.IsNullOrEmpty( content ) ) {
        return new PackageReference[0];
      }
    ...
    public static IEnumerable<PackageReference> Read( TextReader reader ) {
      ...
      return from packageXml in xml.Elements( "package" )
             ...
             select new PackageReference( id, versions, targetFramework, allowedVersions, ParseNullableBool( userInstalled ) );
    }
    ...
    public static void Write( IEnumerable<PackageReference> packages, TextWriter writer, XmlWriterSettings settings = null ) {
