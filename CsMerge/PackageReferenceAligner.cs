            ...
            if ( oldPackagesConfig.IsPackageReference( reference ) ) {
              ...
              if ( oldPackage.Version != newestPackage.Version ) {
                ...
                if ( !File.Exists( referencePath ) ) {
                  ...
                  var loc = new DirectoryInfo( Path.GetDirectoryName( reference.HintPath ) );
                  ...
                  while ( loc != null ) {
                    if ( loc.Name.Equals( newestPackage.TargetFramework, StringComparison.OrdinalIgnoreCase ) ) {
                      relativeCandidate = Path.Combine( loc.FullName, Path.Combine( parts.ToArray() ), assemblyFileName );
                      referencePath = relativeCandidate.Replace( "$(Configuration)", "Release" );
                      if ( File.Exists( referencePath ) ) {
                        break;
                      }
                    }
                  ...
                  if ( relativeCandidate == null ) {
                    throw new ArgumentException( "Failed to fix path: " + reference.HintPath );
                  }

                  newHintPath = relativeCandidate;
